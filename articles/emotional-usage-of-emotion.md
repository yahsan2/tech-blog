...
`Card`コンポーネントの中にデフォルトのスタイルを書いておいて、上書きしたいところだけ、uiを使う側のコンポーネントで書くようにすれば、ずっとスッキリ書くことができます。
...
とはいえ、解像度をどこまで荒くするかは、プロジェクト次第です。場合によっては`fontSize="md"`と書けるようにした方が便利かもしれませんし、別のケースでは`variant="h1"`と指定するようにしたほうが効率的かもしれません。
